 ...
 *
 * Tier 3 — particles: dropped to MINIMAL as a last resort.
 *
 * === RISK NOTE ===
 * Targets `options.simulationDistance()`, `options.entityDistanceScaling()`
 * and `options.particles()`, all following the same OptionInstance<T>
 * pattern. If any of these don't compile against 26.2, open
...
public class PerformanceScaler {
    ...
    private ParticleStatus originalParticleStatus = null;
    ...
    public void tick(Minecraft client) {
        ...
        if (lowFps && ticksSinceLower >= Chlorine.CONFIG.lowerCooldownTicks) {
            if (!simAtFloor) {
                ...
                OptionInstance<ParticleStatus> particleOption = client.options.particles();
                if (originalParticleStatus == null) {
                    originalParticleStatus = particleOption.get();
                }
                if (particleOption.get() != ParticleStatus.MINIMAL) {
                    particleOption.set(ParticleStatus.MINIMAL);
                    ticksSinceLower = 0;
